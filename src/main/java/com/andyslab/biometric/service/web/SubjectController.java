...
public class SubjectController {
     ...
     * result in a conflict status
     * 
     * @return saved subject with subjectId populated
     */
    @RequestMapping(method = RequestMethod.POST, value = "/subject")
    @ResponseBody
    public BiometricSubject create(@RequestBody BiometricSubject subject, HttpServletResponse response,
            UriComponentsBuilder ucBuilder) {
        subject = engine.enroll(subject);
        response.addHeader(HttpHeaders.LOCATION,
                ucBuilder.path("/subject/{subjectId}").buildAndExpand(subject.getSubjectId()).toUriString());
    ...
    @ResponseBody
    public BiometricSubject createOrUpdate(@RequestBody BiometricSubject subject, HttpServletResponse response,
            UriComponentsBuilder ucBuilder) {
